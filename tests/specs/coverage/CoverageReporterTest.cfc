...
component extends="testbox.system.BaseSpec" {
		...
		describe( "CoverageReporter", function(){
			...
			describe( "code coverage processing", function() {
				it( "processCoverageReport - merges coverage data", function() {
					...
					expect( structKeyArray( deserializeJSON( finalResult ) ) )
						.toHaveLength( arrayLen( structKeyArray( deSerializeJSON( initialResult ) ) ), "should merge NOT duplicate coverage data" );
				});
			});
		} );
	}
	...
	function getDummyCodeCoverage(){
		...
		var path = expandPath( "./" );
		var files = [ "CoverageReporterTest.cfc", "CoverageServiceTest.cfc" ];
		var rowN = 0;
		for( var fileName in files ){
			queryAddRow( coverageQuery, {
