<cfcomponent extends="testbox.system.BaseSpec" displayname="MockBox Suite">
	...
	function testMockWithArguments(){
		...
		// Mock
		test
			.$( method = "getSetting", callLogging = true )
			.$args( "AppMapping" )
	...
	function test$Throws(){
		...
		$assert.notThrows(
			target = function(){
				mocked.dontPassThrowToMe();
			}
		);

		$assert.throws(
			target = function(){
				mocked.dontPassThrowToMe( "throw" );
			},
			type    = "MyCustomException",
			message = "My Custom Exception Message"
		);
