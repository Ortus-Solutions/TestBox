...
component {
	...
	function init(){
		...
		// Create Mappings
		fileSystemUtil.createMapping( "testbox", variables.cwd );
	...
	function buildSource(
		...
		// Copy BE to root
		fileCopy( "#variables.projectBuildDir#/box.json", variables.artifactsDir & "/#variables.projectName#" );
		fileCopy( destination, variables.artifactsDir & "/#variables.projectName#/testbox-be.zip" );
		command( "checksum" )
			.params(
	 ...
	 */
	function docs( version = "1.0.0", outputDir = "#variables.cwd#.tmp/apidocs" ){
		// Generate Docs
