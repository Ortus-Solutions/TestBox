...
<div class="border my-1 p-1" id="globalStats">
	...
	<div class="buttonBar">
		<a class=" m-1 btn btn-sm btn-primary float-right" href="#variables.baseURL#" title="Run all the tests">Run All</a>
		<span class=" m-1 btn btn-sm btn-primary float-right" onclick="toggleDebug()" title="Toggle the test debug information">Debug</button>
	</div>
...
<span class="specStatus  m-1 btn btn-sm btn-success" data-status="passed">Pass: #results.getTotalPass()#</span>
<span class="specStatus  m-1 btn btn-sm btn-warning" data-status="failed">Failures: #results.getTotalFail()#</span>
<span class="specStatus  m-1 btn btn-sm btn-danger" data-status="error">Errors: #results.getTotalError()#</span>
<span class="specStatus  m-1 btn btn-sm btn-info" data-status="skipped">Skipped: #results.getTotalSkipped()#</span>
...
<cfloop array="#variables.bundleStats#" index="thisBundle">
	...
	<!--- Bundle div --->
	<div class="border my-1 p-1  bundle" id="bundleStats_#thisBundle.path#" data-bundle="#thisBundle.path#">
		...
		<span class="specStatus  m-1 btn btn-sm btn-success" 	data-status="passed" data-bundleid="#thisBundle.id#">Pass: #thisBundle.totalPass#</span>
		<span class="specStatus  m-1 btn btn-sm btn-warning" 	data-status="failed" data-bundleid="#thisBundle.id#">Failures: #thisBundle.totalFail#</span>
		<span class="specStatus  m-1 btn btn-sm btn-danger" 	data-status="error" data-bundleid="#thisBundle.id#">Errors: #thisBundle.totalError#</span>
		<span class="specStatus  m-1 btn btn-sm btn-info" 	data-status="skipped" data-bundleid="#thisBundle.id#">Skipped: #thisBundle.totalSkipped#</span>
		<span class="rest  m-1 btn btn-sm btn-dark" title="Clear status filters">Reset</span>
		...
		<cfif arrayLen( thisBundle.debugBuffer )>
			<hr>
			<h2>Debug Stream <button class="btn btn-sm btn-primary" onclick="toggleDebug( '#thisBundle.id#' )" title="Toggle the test debug stream"><i class="fas fa-plus-square"></i></button></h2>
	...
	<cfsavecontent variable="local.report">
		...
		<li>
			<a title="Total: #arguments.suiteStats.totalSpecs# Passed:#arguments.suiteStats.totalPass# Failed:#arguments.suiteStats.totalFail# Errors:#arguments.suiteStats.totalError# Skipped:#arguments.suiteStats.totalSkipped#"
			   ...
			   class="#suiteStatsStatusClass#"><strong>+#arguments.suiteStats.name#</strong></a>
			...
			<cfloop array="#arguments.suiteStats.specStats#" index="local.thisSpec">
				...
				<ul>
				<div class="spec #thisSpecStatusClass#" data-bundleid="#arguments.bundleStats.id#" data-specid="#local.thisSpec.id#">
					<li>
						<a href="#variables.baseURL#&testSpecs=#URLEncodedFormat( local.thisSpec.name )#&testBundles=#URLEncodedFormat( arguments.bundleStats.path )#" class="#thisSpecStatusClass#">#local.thisSpec.name# (#local.thisSpec.totalDuration# ms)</a>

						<cfif local.thisSpec.status eq "failed">
							- <strong>#encodeForHTML( local.thisSpec.failMessage )#</strong>
							  <button class="btn btn-sm btn-primary" onclick="toggleDebug( '#local.thisSpec.id#' )" title="Show more information"><i class="fas fa-plus-square"></i></button><br>
							...
							<div class="border my-1 p-1 debugdata" style="display:none;" data-specid="#local.thisSpec.id#">
			...
			<cfif arrayLen( arguments.suiteStats.suiteStats )>
				<cfloop array="#arguments.suiteStats.suiteStats#" index="local.nestedSuite">
					<div class="suite #suiteStatsStatusClass#" data-bundleid="#arguments.bundleStats.id#">
						<ul>
